...
- **別プロジェクトでこのスキルを使う場合**、ローカルに `index.md` が無いことがある。その時は GitHub のコピーを参照する: `https://github.com/bearsunday/BEAR.Kata/blob/1.x/index.md`（raw: `https://raw.githubusercontent.com/bearsunday/BEAR.Kata/1.x/index.md`）。`Source` / `Tests` のパスも同じリポジトリ（BEAR.Kata）の該当ファイルとして読む。
