...
| Fake placement | `tests/Fake/` — `composer.json` maps `BEAR\Kata\` to both `src/` and `tests/` (autoload + autoload-dev), so `fake-hal-api-app` (dev) and `test-hal-api-app` (test) both resolve `BEAR\Kata\Fake\*`. Production (`composer install --no-dev`) does not load `tests/`, keeping the prod artefact free of fake bindings |
...
`BEAR\Kata\Exception\*NotFoundException` family. Only the *primary*
...
- **Use an Input DTO** when any of:
  ...
  Define `BEAR\Kata\Input\<Action>Input` as `final readonly class`
...
```text
Reader Browses By Tag (BEAR\Kata\Hypermedia\ReaderBrowsesByTag)
 ✔ Opens tag list
 ✔ Picks a tag
...
Editor Manages Article (BEAR\Kata\Hypermedia\EditorManagesArticle)
 ✔ Creates an article
