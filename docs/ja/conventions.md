...
| What | Convention |
|------|-----------|
| Namespace root | `BEAR\Kata` |
| Layer ディレクトリ | `src/Entity/`、`src/Query/`、`src/Resource/App/`、`src/Module/`、`src/Service/` |
| Fake の配置 | `tests/Fake/` — `composer.json` は `BEAR\Kata\` を `src/` と `tests/` の両方にマッピング (autoload + autoload-dev) しているので、`fake-hal-api-app` (dev) と `test-hal-api-app` (test) のどちらでも `BEAR\Kata\Fake\*` を解決できます。Production (`composer install --no-dev`) では `tests/` をロードしないので、prod 成果物に fake binding が混入しません |
...
など) で、既存の `BEAR\Kata\Exception\*NotFoundException` 系列に揃えます。
...
- **Input DTO を使う** — 以下のいずれかが当てはまる場合:
  ...
  `BEAR\Kata\Input\<Action>Input` を `final readonly class` として定義し、
...
- 汎用の `LogicException` / `RuntimeException` は使いません。`src/` 由来で
  throw する例外は `BEAR\Kata\Exception\<DomainName>Exception` を定義します。
- Read のエラー (見つからない) は throw せず、`$this->code` 経由で 404 を返します。
- リクエスト検証失敗は `BEAR\Kata\Exception\ValidationException`
...
| 生成された apidoc | `docs/index.html`、`docs/alps.html`、`docs/alps.svg`、`docs/openapi.json`、`docs/llms.txt`、`docs/terms.html`、`docs/terms.md`、`docs/audit.md`、`docs/examples/*`、`docs/schemas/*` |
