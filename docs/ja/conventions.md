...
| MediaQuery result 配置 | `src/Result/*` は `src/Query/*Interface` method から返される型付き Ray.MediaQuery result object の置き場です。これは domain entity ではなく、query execution context や DML metadata を包む object です。このディレクトリは query result 専用に保ち、`src/Query` と `src/Result` が読みやすい対になるようにします |
