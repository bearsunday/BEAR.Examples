...
| **phpstan baseline (1 entry)** | One OAuth provider arg-type widening remains intentionally suppressed — see `phpstan-baseline.neon`. The old `FakeSqlQuery::getRowList()` return-type suppression was removed after the crawl/DataLoader work made the PHPDoc compatible enough for PHPStan. | Wait for the upstream OAuth signature to relax; then drop the entry |
