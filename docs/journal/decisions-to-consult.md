...
| 46 | typed-array DTO field と validation 順序の落とし穴 **resolved (2026-05-28 更新)** | `JsonSchemaInterceptor` (BEAR.Resource 1.31.1) は DTO **hydration の後** に `params:` schema を回すため、当初は `public array $tagIds` に scalar (`tagIds=1`) が来ると JSON Schema が見る前に constructor で `TypeError` → 5xx になる落とし穴があった。Codex adversarial review で発覚 ([job 019dd995-…](https://github.com/bearsunday/MyVendor.Cms/pull/1))。<br /><br />**上流の着地:** BEAR.Resource 1.x-dev / Ray.InputQuery 1.1 で native `array` / `array|null` DTO input が Resource 境界で扱えるようになり、非 array shape は `BEAR\Resource\Exception\ParameterException` (→ 400) として wrap される。<br /><br />**MyVendor.Cms 側:** `ArticleCreateInput::tagIds` を `array $tagIds = []`、`ArticleUpdateInput::tagIds` を `array|null $tagIds = null` に戻し、DTO 側の `mixed` + `is_array` ガードを削除。回帰テスト `tests/Resource/App/ArticleTest.php::testPostRejectsScalarTagIds` / `testPutRejectsScalarTagIds` は 400-class failure として維持。設計記述は `docs/conventions.md` §4 "Native array DTO inputs"。 | クローズ。上流対応済みなので defensive guard は不要。 |
| 47 | README をリファレンス実装の一部として規格化する **resolved (2026-04-30)** | `docs/readme-spec.md` に README spec v1 を定義。トップレベルは `Description` / `Setup` / `Reference` / `Links` の 4 セクションに固定し、詳細はこの下へ収める形にした。`README.md` は spec への準拠例として再構成した。 | クローズ。今後 BEAR.Sunday 系 repo の README を触る時は `docs/readme-spec.md` を入口にする。 |
