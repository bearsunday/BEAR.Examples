...
| # | 項目 | 現状 | 判断 |
|---|------|------|------|
| 45 | `#[JsonSchema(params:)]` で Input DTO を validate できない (上流ギャップ確認済み) | `Article::onPost` / `Article::onPut` / `Auth::onPost` を `#[Input] <Dto>` 化したが、`JsonSchemaInterceptor` は flat scalar 引数を前提とした実装で DTO 引数を見ない。`var/json_validate/article_create.json` / `article_update.json` / `auth_exchange.json` は **削除せず** 残してあり、各メソッドの直上に `/** TODO(input-query+json-schema): ... */` を貼って欠落を可視化している。`docs/conventions.md` §4 にも「Input DTO 採用部分は現状 schema 適用なし」と明記。<br /><br />**Codex 診断 (2026-04-26, セッション `019dc7d3-23ce-7c02-be70-78dacb6ab2b7`):** ベンダソースを直接読んで 3 点確認した。(1) `BEAR\Resource\InputParam` がメソッド呼び出し前に DTO を materialize する。(2) その後 `JsonSchemaInterceptor::getNamedArguments()` は `['input' => FooInput]` を validate しに行く — flat な request array ではない。(3) スキーマを `input` で wrap するワークアラウンドは `justinrainbow/json-schema` validator が readonly DTO プロパティで crash するため不可。要するに **アプリ側だけの小さな修正では成立しない上流ギャップ**。<br /><br />**副産物:** 当初消えた commit `4c87344` の `JsonSchemaNotFoundException` の根本原因も判明。`json_validate_dir` の root は正しかった (`var/json_validate`)。問題は `#[JsonSchema(params:)]` を付けた時 `schema:` (response schema) 引数が空のままだったため、interceptor が response 側でも `json_schema_dir . '/'` (ディレクトリパス) を `validateFileExists()` に渡して落ちた。Auth は GET で auth URL 文字列を返すだけで response schema が無いので落ちる構図。仮に将来 params を再アタッチするなら response schema もセットで用意する必要がある。 | 上流 (BEAR.Resource: `JsonSchemaInterceptor` を `#[Input]` 認識にする、または `InputParam` の materialize 順序を変える) でフィクスが入るまで現状維持。**現コミット段階では params: を再アタッチしない。** Article/Auth の docblock からは "TODO" を外し、本ジャーナル #45 へのリンクと根本原因要約に差し替える。BEAR.ApiDoc 側 (issue #76 / 別 PR) で `#[Input]` DTO を introspect してドキュメントとスキーマを生成する経路で補う方が筋が良い (codex は別タスクで継続予定)。 |
