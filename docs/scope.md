...
| `composer demo:async` | BEAR.Async Article embed timing demo (sync vs parallel `#[Embed]`); Docker recommended |
| `composer docker:async-demo` | Same demo in PHP ZTS + `ext-parallel` Docker runtime |
| `composer docker:async-test` | Async smoke test in Docker |
...
| BEAR.Async entrypoint swap | `bin/async.php` enables parallel `#[Embed]` execution without changing `Article::onGet` or `AppModule` |
...
| D3 | ~~Async `#[Embed]` parallelization~~ | Implemented as a `bin/async.php` entrypoint that overlays the BEAR.Async parallel runtime on the unchanged `AppModule` | Use `composer docker:async-demo` / `docker:async-test`; local PHP without `ext-parallel` skips the async test |
