...
|---|------|--------------|----------------------|
| D1 | `#[CacheableResponse]` on list reads + `#[Purge]` on writes (PR-C2) | **Partial — landed for list resources only.** `Articles` / `Categories` carry class-level `#[CacheableResponse]`; `Article` / `Category` writes carry `#[Purge(uri: 'app://self/{collection}')]`. Entity-level `#[CacheableResponse]` is intentionally skipped because `DonutCommandInterceptor` mutates `onDelete` responses (204 → 404) by re-running `onGet` on a deleted entity, and embedded resources collide with `CmsQiqRenderer` in the html context | Pipeline verified in `tests/Resource/App/CacheTest.php` (asserts `try-donut-view` / `put-donut` / `save-etag` / `purge-query-repository` in `RepositoryLogger`). Optional follow-up: explore entity-level caching once the delete-mutation upstream behavior is clarified |
