...
Generated commands under `bin/cli/` cover read and write for every scalar-parameter resource method: `article-show`, `article-list`, `article-delete`, `author-show`/`-add`/`-update`, `category-list`/`-show`/`-add`/`-update`/`-delete`, `tag-list`/`-show`/`-add`/`-delete`, `media-show`/`-add`/`-delete`. `Article::onPost` / `onPut` are intentionally **not** CLI-exposed: they take an `#[Input]` DTO (`ArticleCreateInput` / `ArticleUpdateInput`) and `bear/cli` only maps `#[Option]`-annotated scalar parameters, so a DTO-receiving method would generate a no-op command. Keeping it CLI-only on the scalar variants preserves the DTO-vs-scalar contrast documented in the "By design" section below.
